  ...
  properties: {
    ...
    checkboxes: { type: 'array', items: { type: 'object', additionalProperties: false,
      properties: {
        ...
        label_right: { type: 'string' }, row: { type: 'number' },
...
const CHECKS = { type: 'object', additionalProperties: false,
  properties: { chosen: { type: 'array', items: { type: 'object', additionalProperties: false,
    properties: { id: { type: 'string' }, cx: { type: 'number' }, cy: { type: 'number' },
      reason: { type: 'string' } }, required: ['id', 'cx', 'cy'] } } },
  required: ['chosen'] }
...
(id, cx, cy_pdf, label_right, row=integer row index grouping boxes on the same line).`,
...
label clearly implies otherwise). Return chosen[] with the picked box id, its cx, and
cy (use the box's cy_pdf).`,
...
const checkItems = checks.chosen.map(c => ({ page: 0, kind: 'check',
  cx: c.cx, cy: c.cy, size: 10, font: 'thai' }))
